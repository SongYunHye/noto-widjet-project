...
function TodoItem({ todo, onToggle, onToggleFavorite, onDelete, onEdit, isDragging }) {
  // 텍스트 길이가 30자 이상이면 전체 너비 사용
  const isLongText = todo.text.length > 30;
  
  return (
    <div 
      className={`todo-item ${todo.completed ? 'completed' : ''} ${!todo.completed ? 'draggable' : ''} ${isLongText ? 'todo-item-long' : ''} ${isDragging ? 'is-dragging' : ''}`}
    >
      <div className="drag-handle" title="드래그하여 순서 변경" style={{ opacity: todo.completed ? 0 : 1 }}>
      ...
      <div className="todo-actions">
        ...
        >
          {todo.isFavorite ? (
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="#ff6b9d" stroke="#ff6b9d" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
